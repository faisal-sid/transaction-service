...
@when("an account {account_number} withdraws {amount:d}")
def withdraw(context, account_number, amount):
    context.events_in.produce(dict(
        id='1987b482-5e66-4b7f-bd95-ac76f27ed85d',
        accountNumber=account_number,
        amount=amount,
...
@then("account {account_number} should not be allowed to withdraw")
def do_not_withdraw(context, account_number):
    pass  # Intentional no-op
...
# @then("account {account_number} should have a balance of {balance:d}")
# def balance(context):
    
#     raise NotImplementedError(u'STEP: Then account 12345678 should have a balance of 500')